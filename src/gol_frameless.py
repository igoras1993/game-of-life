...
class GUI(QWidget, Ui_Main_Widget):
    def __init__(self, parent=None):
        ...
        #moving window
        self.mousePressEvent = self.mainPressEvent
        self.oldWindowPos = None
        self.mouseMoveEvent = self.mainMoveEvent
    ...
    def mainPressEvent(self, event):
        self.oldWindowPos = event.globalPos()

    def mainMoveEvent(self, event):
        if self.oldWindowPos is None:
            self.oldWindowPos = event.globalPos()
        delta = event.globalPos() - self.oldWindowPos
